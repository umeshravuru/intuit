...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Transform data for chart if needed, but the backend structure is pretty flat already
      // Backend returns: period_start (date string), stockCode, period_predicted_sales (number)
      // Grouping by date might be needed if we want multiple lines for different StockCodes
      // For now, let's just plot what we get. If there are multiple StockCodes, it might look messy without grouping.
      // Let's optimize: The SQL limits to top 5 categories. So per date, we have up to 5 points.
      // Recharts needs data like: { date: '2023-01-01', stockA: 100, stockB: 200 }
      ...
      rawData.forEach(item => {
        ...
        pivotedData[date][item.stockCode] = item.monthlyPredictedSales;
      });

      const chartData = Object.values(pivotedData).sort((a, b) => new Date(a.date) - new Date(b.date));

      // Extract unique stock codes for lines
      const stockCodes = [...new Set(rawData.map(item => item.stockCode))];
